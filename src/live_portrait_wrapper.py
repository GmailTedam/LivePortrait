...
class LivePortraitWrapper(object):
    ...
    def __init__(self, inference_cfg: InferenceConfig):
        ...
        log(
            f"Load appearance_feature_extractor from {osp.realpath(inference_cfg.checkpoint_F)} done."
        )
        # init M
        ...
        log(
            f"Load motion_extractor from {osp.realpath(inference_cfg.checkpoint_M)} done."
        )
        # init W
        ...
        log(
            f"Load warping_module from {osp.realpath(inference_cfg.checkpoint_W)} done."
        )
        # init G
        ...
        log(
            f"Load spade_generator from {osp.realpath(inference_cfg.checkpoint_G)} done."
        )
        # init S and R
        if inference_cfg.checkpoint_S is not None and osp.exists(
            ...
            log(
                f"Load stitching_retargeting_module from {osp.realpath(inference_cfg.checkpoint_S)} done."
            )
        else:
        ...
        if self.compile:
            torch._dynamo.config.suppress_errors = (
                True  # Suppress errors and fall back to eager execution
            )
    ...
    def prepare_source(self, img: np.ndarray) -> torch.Tensor:
        ...
        h, w = img.shape[:2]
        if (
            h != self.inference_cfg.input_shape[0]
            or w != self.inference_cfg.input_shape[1]
        ):
            x = cv2.resize(
                img,
                (self.inference_cfg.input_shape[0], self.inference_cfg.input_shape[1]),
            )
        else:
            x = img.copy()
    ...
    def calc_combined_eye_ratio(self, c_d_eyes_i, source_lmk):
        ...
        c_d_eyes_i_tensor = (
            torch.Tensor([c_d_eyes_i[0][0]]).reshape(1, 1).to(self.device)
        )
        # [c_s,eyes, c_d,eyes,i]
        combined_eye_ratio_tensor = torch.cat(
            [c_s_eyes_tensor, c_d_eyes_i_tensor], dim=1
        )
    ...
    def calc_combined_lip_ratio(self, c_d_lip_i, source_lmk):
        ...
        c_d_lip_i_tensor = (
            torch.Tensor([c_d_lip_i[0]]).to(self.device).reshape(1, 1)
        )  # 1x1
        # [c_s,lip, c_d,lip,i]
...
class LivePortraitWrapperAnimal(LivePortraitWrapper):
    ...
    def __init__(self, inference_cfg: InferenceConfig):
        ...
        # init F
        self.appearance_feature_extractor = load_model(
            inference_cfg.checkpoint_F_animal,
            model_config,
            self.device,
            "appearance_feature_extractor",
        )
        log(
            f"Load appearance_feature_extractor from {osp.realpath(inference_cfg.checkpoint_F_animal)} done."
        )
        # init M
        self.motion_extractor = load_model(
            inference_cfg.checkpoint_M_animal,
            model_config,
            self.device,
            "motion_extractor",
        )
        log(
            f"Load motion_extractor from {osp.realpath(inference_cfg.checkpoint_M_animal)} done."
        )
        # init W
        self.warping_module = load_model(
            inference_cfg.checkpoint_W_animal,
            model_config,
            self.device,
            "warping_module",
        )
        log(
            f"Load warping_module from {osp.realpath(inference_cfg.checkpoint_W_animal)} done."
        )
        # init G
        self.spade_generator = load_model(
            inference_cfg.checkpoint_G_animal,
            model_config,
            self.device,
            "spade_generator",
        )
        log(
            f"Load spade_generator from {osp.realpath(inference_cfg.checkpoint_G_animal)} done."
        )
        # init S and R
        if inference_cfg.checkpoint_S_animal is not None and osp.exists(
            inference_cfg.checkpoint_S_animal
        ):
            self.stitching_retargeting_module = load_model(
                inference_cfg.checkpoint_S_animal,
                model_config,
            ...
            log(
                f"Load stitching_retargeting_module from {osp.realpath(inference_cfg.checkpoint_S_animal)} done."
            )
        else:
            self.stitching_retargeting_module = None
