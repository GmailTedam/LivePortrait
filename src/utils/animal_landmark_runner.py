...
class XPoseRunner(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.model = self.load_animal_model(
            model_config_path, model_checkpoint_path, self.device
        )
        self.timer = Timer()
        ...
        try:
            ...
            with open(f"{embeddings_cache_path}_68.pkl", "rb") as f:
                self.ins_text_embeddings_68, self.kpt_text_embeddings_68 = pickle.load(
                    f
                )
            print("Loaded cached embeddings from file.")
        except Exception:
            raise ValueError(
                "Could not load clip embeddings from file, please check your file path."
            )
    ...
    def get_unipose_output(
        ...
        if len(keypoint_text_prompt) == 9:
            ...
            ins_text_embeddings, kpt_text_embeddings = (
                self.ins_text_embeddings_9,
                self.kpt_text_embeddings_9,
            )
        elif len(keypoint_text_prompt) == 68:
            # torch.Size([1, 512]) torch.Size([68, 512])
            ins_text_embeddings, kpt_text_embeddings = (
                self.ins_text_embeddings_68,
                self.kpt_text_embeddings_68,
            )
        else:
        ...
        target = {
            ...
            "kpts_embeddings_text": torch.cat(
                (
                    ...
                    torch.zeros(
                        100 - kpt_text_embeddings.shape[0], 512, device=self.device
                    ),
                ),
                dim=0,
            ),
            "kpt_vis_text": torch.cat(
                (
                    torch.ones(kpt_text_embeddings.shape[0], device=self.device),
                    torch.zeros(100 - kpt_text_embeddings.shape[0], device=self.device),
                ),
                dim=0,
            ),
        }
    ...
    def run(
        ...
        boxes_filt, keypoints_filt = self.get_unipose_output(
            image,
            instance_text_prompt,
            keypoint_text_prompt,
            box_threshold,
