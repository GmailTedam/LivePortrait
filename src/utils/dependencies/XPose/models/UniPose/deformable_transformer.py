...
class DeformableTransformer(nn.Module):
    ...
    def forward(
        ...
        if self.two_stage_type == "standard":
            ...
            if self.binary_query_selection:
                topk_logits = self.binary_query_selection_layer(output_memory).squeeze(
                    -1
                )
            else:
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        self.kpt_index = [
            x
            for x in range(50 * (self.num_body_points + 1))
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if layer_id == self.num_box_decoder_layers - 1:
                ...
                class_unselected = self.class_embed[layer_id](
                    output.transpose(0, 1), text_dict
                ...
                new_output_for_keypoint = keypoint_embed[None, :, :, :].repeat(
                    new_output_for_box.shape[0], 1, 1, 1
                )
            ...
            if layer_id >= self.num_box_decoder_layers:
                ...
                output_bbox_norm = output[effect_num_dn:][
                    0 :: (self.num_body_points + 1)
                ]
                reference_before_sigmoid_bbox_dn = reference_before_sigmoid[
                    :effect_num_dn
                ]
                reference_before_sigmoid_bbox_norm = reference_before_sigmoid[
                    effect_num_dn:
                ][0 :: (self.num_body_points + 1)]
                ...
                outputs_unsig_norm = (
                    delta_unsig_norm + reference_before_sigmoid_bbox_norm
                )
