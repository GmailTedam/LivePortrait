...
class LivePortraitPipelineAnimal(object):
    def __init__(self, inference_cfg: InferenceConfig, crop_cfg: CropConfig):
        self.live_portrait_wrapper_animal: LivePortraitWrapperAnimal = (
            LivePortraitWrapperAnimal(inference_cfg=inference_cfg)
        )
    ...
    def make_motion_template(self, I_lst, **kwargs):
        ...
        for i in track(
            range(n_frames),
            description="Making driving motion templates...",
            total=n_frames,
        ):
    ...
    def execute(self, args: ArgumentConfig):
        ...
        if flag_load_from_template:
            ...
            I_d_lst = self.live_portrait_wrapper_animal.prepare_videos(
                driving_rgb_crop_256x256_lst
            )
            driving_template_dct = self.make_motion_template(
                I_d_lst, output_fps=output_fps
            )
            ...
            raise Exception(
                f"{args.driving} not exists or unsupported driving info types!"
            )
        ...
        for i in track(range(n_frames), description="🚀Animating...", total=n_frames):
            ...
            if (
                ...
                I_p_pstbk = paste_back(
                    I_p_i, crop_info["M_c2o"], img_rgb, mask_ori_float
                )
        ...
        wfp = osp.join(
            args.output_dir, f"{basename(args.source)}--{basename(args.driving)}.mp4"
        )
