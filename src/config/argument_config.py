...
class ArgumentConfig(PrintableConfig):
    ########## input arguments ##########
    source: Annotated[str, tyro.conf.arg(aliases=["-s"])] = make_abs_path(
        "../../assets/examples/source/s0.jpg"
    )  # path to the source portrait (human/animal) or video (human)
    driving: Annotated[str, tyro.conf.arg(aliases=["-d"])] = make_abs_path(
    ...
    server_port: Annotated[int, tyro.conf.arg(aliases=["-p"])] = (
        8890  # port for gradio server
    )
    share: bool = False  # whether to share the server to public
    server_name: Optional[str] = (
        "127.0.0.1"  # set the local server name, "0.0.0.0" to broadcast all
    )
    flag_do_torch_compile: bool = (
        False  # whether to use torch.compile to accelerate generation
    )
