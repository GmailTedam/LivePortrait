...
class CropConfig(PrintableConfig):
    ...
    xpose_config_file_path: str = make_abs_path(
        "../utils/dependencies/XPose/config_model/UniPose_SwinT.py"
    )
    xpose_embedding_cache_path: str = make_abs_path("../utils/resources/clip_embedding")

    xpose_ckpt_path: str = make_abs_path(
        "../../pretrained_weights/liveportrait_animals/xpose.pth"
    )
    ...
    scale_crop_driving_video: float = (
        2.2  # 2.0 # scale factor for cropping driving video
    )
